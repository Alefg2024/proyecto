					...
					<?php if (count($operations) > 0): ?>
						...
						<script type="text/javascript">
							function thePDF() {
								...
								doc.setFontSize(18);
								doc.text("TOTAL : <?php echo Core::$symbol; ?> <?php echo number_format($supertotal, 2, '.', ','); ?> <?php if (isset($_GET['user_id']) && $_GET['user_id'] != '') {
																																			$uy = UserData::getById($_GET["user_id"]);
																																			echo ' - Comision (' . $uy->comision . '%): ' . Core::$symbol . ' ' . number_format($supertotal * ($uy->comision / 100), 2, '.', ',');
